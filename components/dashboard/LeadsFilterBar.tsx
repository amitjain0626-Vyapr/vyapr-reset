...
const STATUSES_OK = ["success", "paid", "captured"];
...
export default async function RoiTracker() {
  ...
  const [today, last7, last30, mtd, lmtd] = await Promise.all([
    sumForRange(supabase, user.id, todayStart.toISOString()),
    sumForRange(supabase, user.id, last7Start.toISOString()),
    sumForRange(supabase, user.id, last30Start.toISOString()),
    sumForRange(supabase, user.id, monthStart.toISOString()),
    sumForRange(supabase, user.id, prevMonthStart.toISOString(), todayStart.toISOString()),
  ]);
