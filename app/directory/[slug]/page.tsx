...
// Robust-ish parser: category = first segment before first hyphen,
// city = everything after first hyphen (allows multi-word cities via hyphens).
function parseSlug(slug: string): { category: string; city: string } {
  const trimmed = (slug || "").trim();
  const i = trimmed.indexOf("-");
  if (i === -1) {
    return { category: trimmed || "service", city: "city" };
  }
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { category, city } = parseSlug(params.slug);
  ...
  const canonical = `/directory/${params.slug}`;
...
export default async function DirectoryPage({ params }: PageProps) {
  ...
  // If you already have a provider fetch + grid, paste it above the FAQ block.
  // For Step 1, we focus on dynamic FAQs + JSON-LD only.
