...
function buildWaUrl({ phone, whatsapp, display_name, slug }: any) {
  ...
  if (!raw) return "";
  const msg = encodeURIComponent(`Hi${display_name ? " " + display_name : ""}, I'd like to book a slot via Vyapr (${SITE}/book/${slug}).`);
    ...
    <main className="mx-auto max-w-3xl px-4 py-10 space-y-8">
      ...
      <section className="rounded-2xl border bg-white p-5">
        ...
        <div className="flex flex-wrap gap-3">
          <a href={waUrl || "#"} aria-disabled={!waUrl}
             className={`inline-flex items-center rounded-full border px-4 py-2 text-sm ${waUrl ? "border-emerald-600 hover:shadow-sm" : "border-gray-300 opacity-50 cursor-not-allowed"}`}
