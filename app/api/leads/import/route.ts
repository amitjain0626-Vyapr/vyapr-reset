...
export async function POST(req: Request) {
  try {
    ...
    if (!file) return NextResponse.json({ ok: false, error: "missing_csv_file" }, { status: 400 });
    ...
    if (!grid.length) return NextResponse.json({ ok: false, error: "empty_csv" }, { status: 400 });
    ...
    if (!rows.length) return NextResponse.json({ ok: false, error: "no_valid_rows", skipped }, { status: 400 });
    ...
    // Telemetry (use your existing casing + ms timestamp like create route)
    const eventRow = {
      event: "lead.imported",
      ts: Date.now(),
      provider_id: prov.id,
    ...
    return NextResponse.json({ ok: true, provider_slug: prov.slug, imported: count, skipped });
  } catch (e: any) {
