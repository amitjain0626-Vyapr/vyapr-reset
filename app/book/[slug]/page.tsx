...
type Provider = {
  ...
  faqs?: any;                   // JSON or JSON string [{q,a}]
  bio?: string | null;
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 space-y-8">
      ...
      <section className="grid grid-cols-1 gap-3 text-sm">
        ...
        {toNumber(provider?.latitude) !== undefined && toNumber(provider?.longitude) !== undefined ? (
          <div>
            <span className="font-medium">Geo:</span>{" "}
            <span className="tabular-nums">
              {toNumber(provider?.latitude)?.toFixed(6)}, {toNumber(provider?.longitude)?.toFixed(6)}
            </span>
          </div>
        ) : null}
      </section>
      ...
      <section className="space-y-2">
        ...
        {normalizeFaqs(provider?.faqs)?.length ? (
          <div className="space-y-2">
            {normalizeFaqs(provider?.faqs)!.map((f, i) => (
