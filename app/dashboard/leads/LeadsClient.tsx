...
export default function LeadsClient({ initial }: { initial: LeadRow[] }) {
  ...
  async function fetchLeadsOnce() {
    try {
      const url = `/api/leads/list?t=${Date.now()}`; // hard bust
      ...
      // fail-open: ignore errors, try again on next tick
    }
  }

  // Reliable auto-refresh: every 5 seconds
  useEffect(() => {
    fetchLeadsOnce();                  // immediate
    const iv = setInterval(fetchLeadsOnce, 5000);
    // also refresh when tab regains focus (common macOS case)
    const onVis = () => { if (!document.hidden) fetchLeadsOnce(); };
  ...
  return (
    <>
      ...
      {count === 0 ? (
        <div className="rounded-xl border p-8 text-center">
          ...
          <div className="mt-1 text-sm text-neutral-600">
            This view auto‑refreshes every 5s. Submit your microsite form to see leads here.
          </div>
        </div>
      ) : (
      ...
      <div className="text-xs text-neutral-500">
        Auto‑refresh: 5s • No schema changes • Cookie session preserved
      </div>
    </>
  );
}
